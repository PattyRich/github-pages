...
class Osrs extends React.Component {
	...
	async addIcons(mode, loot = null) {
		...
		if (mode === 'create') {
			...
			if (data.pet) { promiseArray.push(getIcon(data.pet.name)) }
			await Promise.all(promiseArray)
			this.setState({ 'icons': iconClone })
			return
		}
	...
	async go() {
		...
		if (this.state.mode === 'create') { this.completion(this.state.mode); }
	...
	async graphSimulation(skipGraph = false) {
		...
		for (let i = 0; i < this.state.simulations; i++) {
			...
			if (!best || x[x.length - 1].kc < best[best.length - 1].kc) { best = x }
			if (!worst || x[x.length - 1].kc > worst[worst.length - 1].kc) { worst = x }
		}
	...
	render() {
		...
		return (
			<div className="route-dark-bg osrs-theme" style={{ padding: '20px' }}>
				<div className="box">
					<div>
						{bossModes.map((boss) => (
							<span key={boss.value}>
								<input type="radio" value={boss.value} name="boss-mode" checked={this.state.mode === boss.value} onChange={this.onChangeValue} /> {boss.label}&nbsp;
							</span>
						))}
					</div>
					<div>
						{this.state.mode === 'clues' &&
							cluesLvl.map((clue, i) => (
								<span key={clue}>
									<input type="radio" value={cluesLvl[i]} name="clue-level" checked={this.state.clue === cluesLvl[i]} onChange={(e) => this.onChangeValueInput('clue', e)} />
					...
					{this.state.mode === 'create' &&
						<div style={{ margin: '30px' }}>
							<div style={{ padding: '10px', margin: '10px 0' }}>
								...
								&nbsp; Boss name: <input type="text" value={this.state.createData.bossName} onChange={(e) => this.changeCreateData('bossName', e.target.value)} />
								&nbsp; Load previous boss:
								<select value='' onChange={(e) => this.selectBoss(e.target.value)}>
									<option value=""></option>
									{this.state.bosses.map((boss) => (
										<option key={boss.bossName} value={boss.bossName}>{boss.bossName}</option>
									))}
								</select>
							</div>
							...
							{this.state.createData.items.map((item, index) => (
								<div key={index}>
									Item {index + 1}: Name: <input type="text" value={item.name} onChange={(e) => this.changeCreateData('name', e.target.value, index)} />
									&nbsp; Rate: <input type="text" value={item.rate} onChange={(e) => this.changeCreateData('rate', e.target.value, index)} />
								</div>
							))}
							{this.state.pets &&
								<div>
