        ...
        <div className="draft-wrap route-dark-bg">
          ...
          {this.state.editMode && (
            <div className="flex-center" style={{ alignItems: 'center' }}>
              <Button
                click={() => this.changeTeamCount(false)}
                variant="outline-primary"
                text={'-'}
              />
              <span style={{ margin: '0px 10px 0px 10px' }}> Add or Remove Teams </span>
              <Button
                click={() => this.changeTeamCount(true)}
                variant="outline-primary"
                text={'+'}
              />
            </div>
          ...
          <div className="draft-teams">
            {this.state.teams.map((team, i) => {
              return (
                <Dustbin
                  dropped={this.movePlayer}
                  editMode={this.state.editMode}
                  editName={(e) => this.editName(e, i)}
                  key={i + team.name}
                  team={team}
                />
              );
            ...
            <div className="flex">
              ...
              <div className="flex" style={{ marginLeft: '15px', width: '100%', flexWrap: 'wrap' }}>
                {this.state.players
                  .split(',')
                  .filter((name) =>
                    name.toLocaleLowerCase().includes(this.state.filter.toLowerCase())
                  )
