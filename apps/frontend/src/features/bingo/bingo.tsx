...
type JoinPasswordTitle = 'admin' | 'general';
type NumericBingoStateKey = 'columns' | 'rows' | 'teams';
...
type BingoStateChange = Partial<BingoState>;
...
function Bingo({ screenSkip }: BingoProps) {
  ...
  useEffect(() => {
    if (
      ...
      const recentBoards = parseRecentBoards(localStorage.getItem('recentBoards'));
      setState((currentState) => ({
        ...currentState,
  ...
  function changeNum(value: number, target: NumericBingoStateKey) {
  ...
  function removeRecent(indexToRemove: number) {
    const recentBoards = (state.recentBoards || []).filter(
      (_thing, index) => index !== indexToRemove
    );
  ...
  async function auth(recentSkip: RecentBoard | false = false) {
    if (recentSkip && recentSkip.boardName) {
      const password = String(recentSkip.password ?? '');
      const obj: BingoNavigationState = {
        generalPassword: password,
        adminPassword: password,
        privilege: recentSkip.privilege ?? recentSkip.priv,
        boardName: recentSkip.boardName,
      };
    ...
    const navigationState: BingoNavigationState = {
            ...
            <div className="join-board-grid">
              <Surface as="section" className="join-board-panel" variant="glass">
                ...
                <div className="join-board-actions">
                  <Button variant="success" click={() => void auth()} text="Join Board"></Button>
                </div>
              </Surface>
...
function parseRecentBoards(value: string | null): RecentBoard[] {
  if (!value) return [];
  try {
