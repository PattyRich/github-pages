...
export function loot(
  rolls,
  place,
  options = { points: 30000, runCompletion: false, pets: true, cms: false, teamSize: 1 }
) {
  return new Promise((resolve, reject) => {
    ...
    loadLootModule().then((datax) => {
      ...
      if (teamActivites.includes(data.name)) {
        ...
        if (data.name === 'nex') data.pet.rate *= options.teamSize;
      }
      ...
      if (!data.chance) {
        ...
        if (data.eq) {
          ...
          data.pet.rate = Math.round(
            1 / (eval(data.pet.rate.replaceAll('x', options.invocation)) / 100)
          );
        } else {
          let sum = 0;
          data.items.forEach((item) => {
...
function cleanClueRewards(rewards) {
  ...
  rewards.forEach((reward) => {
    let index = rewardsCleaned.findIndex((item) => {
