...
type SettingsKey = (typeof SETTINGS_KEYS)[number]['key'];

interface SettingsModalProps {
  handleClose: () => void;
}

export default function SettingsModal({ handleClose }: SettingsModalProps) {
  const [settings, setSettings] = useState(
    () =>
      Object.fromEntries(SETTINGS_KEYS.map(({ key }) => [key, getStoredBool(key)])) as Record<
        SettingsKey,
        boolean
      >
  );
