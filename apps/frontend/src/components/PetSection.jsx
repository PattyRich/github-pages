...
export default function PetSection({ section, pets, petInfo, path }) {
  ...
  return (
    <div className="pet-section">
      ...
      <div
        style={
          WRAP_SECTIONS.includes(section)
            ? { flexWrap: 'wrap', alignSelf: 'center', width: '80%' }
            : {}
        }
      ...
      >
        {pets.map((pet) => {
          ...
          return (
            <div
              key={pet}
              className="pet-individual"
              style={{ filter: petData.obtained ? 'brightness(100%)' : 'brightness(40%)' }}
            >
              <div className="pet-image-container-ofc">
                <img
                  className={`pet-image-ofc ${scale ? 'scaled' : ''}`}
                  src={assetUrl(`${path}/${pet}.png`)}
                  alt={pet}
                />
              </div>
