...
**CI/CD** — GitHub Actions runs on push to `main`. The frontend workflow typechecks, builds a release image, copies the complete build into a versioned directory in `frontend_assets`, and atomically switches `/srv/frontend/current`. The running Nginx process is not replaced. Hashed assets are retained in a shared directory so requests crossing the switch remain valid. Site configuration changes use `nginx -s reload`, which starts new workers before gracefully retiring the old workers. The first rollout of this architecture recreates Nginx once to attach the shared volume.
