...
**CI/CD** — GitHub Actions runs on push to `main`. The frontend/Nginx workflow typechecks, then SSHes into the server to rebuild, validate, and restart the Nginx image. The backend workflow SSHes in to rebuild and restart only `api` and `worker`, leaving Nginx alone to avoid edge blips during routine backend deploys. During the first transition, workflows skip starting Docker Nginx while the host Nginx system service is still active. A weekly scheduled workflow prunes dangling images and updates non-edge containers; Nginx rebuilds are applied by the frontend/Nginx workflow.
